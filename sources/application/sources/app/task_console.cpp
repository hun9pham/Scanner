...
int8_t csDbg(uint8_t *argv) {
	...
	case 'b': {
		APP_DBG(TAG, "Control engine forward in 2s");
		ctrlEngine1State(ROTATE_FORWARD);
		ctrlEngine2State(ROTATE_FORWARD);
		delayMillis(2000);
		ctrlEngine1State(STOPPING);
		ctrlEngine2State(STOPPING);
	}
	break;

	case 'c': {
		APP_DBG(TAG, "Control engine backward in 2s");
		ctrlEngine1State(ROTATE_BACKWARD);
		ctrlEngine2State(ROTATE_BACKWARD);
		delayMillis(2000);
		ctrlEngine1State(STOPPING);
		ctrlEngine2State(STOPPING);
	}
	...
	default: {
		...
		APP_PRINT("  b: Engine forward\n");
		APP_PRINT("  c: Engine backward\n");
		APP_PRINT("  d: ...\n");	
	}
	break;
	}
