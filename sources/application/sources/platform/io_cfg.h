...
#define IO_LED_DIR_PIN				            ( GPIO_Pin_8 )
#define IO_LED_DIR_PORT				            ( GPIOB )
...
/* Enumarics -----------------------------------------------------------------*/
typedef enum eEngineControlStatus {
    STOPPING = 0, /* Động cơ đứng yên */
    ROTATE_FORWARD, /* Động cơ quay tiến */
    ROTATE_BACKWARD, /* Động cơ quay lui */
} EngineStateCtl_t;
...
extern void makeBeepSound(uint16_t timMillis);
...
extern void ctrlEngine1State(EngineStateCtl_t state);
extern void ctrlEngine2State(EngineStateCtl_t state);
