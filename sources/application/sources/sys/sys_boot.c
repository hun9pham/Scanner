...
/* Extern variables ----------------------------------------------------------*/
extern uint32_t __ld_boot_share_data_flash;

/* Private variables ---------------------------------------------------------*/
static sysBoot_t Bootloader;
...
void getSysBoot(sysBoot_t* sysBoot) {
	memcpy((uint8_t*)sysBoot, (uint8_t*)&Bootloader, sizeof(sysBoot_t));
}

uint8_t setSysBoot(sysBoot_t* sysBoot) {
	/* Update RAM object */
	memcpy((uint8_t*)&Bootloader, (uint8_t*)sysBoot, sizeof(sysBoot_t));
